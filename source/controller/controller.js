...
*/
	...
	*/
	...
	process: function(name, node, container) {
		...
		//console.log('ui.builder._initElement()', name, typeOf(node), opts);

		if (typeOf(node) == 'object') {
			//console.log('starting building',node);
			// define class to intaciate

			//console.log(ui.builder.opts.clss.define);
			...
			if (clss.length > 1) {
				...
				//console.log('builded', obj);

				//console.log('builded ', node.name, node.node, obj);
			...
			node.each(function(sub, i) {
				//console.log('that is a list', name, sub, container);
...
ui.request = {
	...
	get: function(url) {
		// console.log('ui.request.get()', url);
		return new Request({
			url: url,
			onComplete: function(json, text) {
				//console.log('ui.request.get() ...ok', json, text);
				this.fireEvent('ok', json, text);
			},
			onError: function(text, error) {
				//console.log('ui.request.get() ...error', text);
				this.fireEvent('error', json, text);
			},
			onFailure: function(xhr) {
				//console.log('ui.request.get() ...fail', xhr);
	...
	html: function(url) {
		...
		return new Request.HTML({
			...
			onSuccess: function(json, text) {
				//console.log('ui.request.get() ...success', json, text);
			...
			onFailure: function(xhr) {
				//console.log('ui.request.get() ...failure', xhr);
			}
		}).get();
	},
...
ui.controller.element = {
	...
	*/
	register: function(object) {
		...
		//console.log('ui.controller.register()',object.element,object);
